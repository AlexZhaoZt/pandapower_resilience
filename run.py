...
from pandapower.pypower.idx_gen import PG, GEN_BUS, PMAX, GEN_STATUS
...
# def _island_check(net):
#     """ 
#     Checks if the net has islands (isolated sub-networks). Note that this function
#     does not assign slack buses to the islands, which is handled in "_add_slack".
#     If an island cannot be assigned a slack bus, mark it as out-of-service.    

#     Args:
#         net: pandapower network

#     Returns:
#         bool: whether there is at least one island in the network (grid is made of at least two 
#                 connected components)
#         bool: whether any island is declared out-of-service
#         int:  number of islands in the network
#     """
...
#     if conn_B[0] == 1:
#         return False, num_out_of_service, conn_B[0]
...
#                 bus[i_island, 1] = NONE
#                 num_out_of_service += len(i_island)

#     return True, num_out_of_service, conn_B[0]

def rundceither(net):
    """
    performs island check and runs either dc powerflow or dc optimal powerflow, depending 
    on if there are islands in the network. If any island has been declared out-of-service,
    performs connectivity check to ensure they are handled. 

    additionally, set num_islands attribute for the network. 

    Args:
        net: pandapower network
    """
    # rundcpp(net, check_connectivity=True)
    
    # if net.num_islands == 1:
    #     rundcpp(net, check_connectivity=True)
    # else:
    rundcopp(net, check_connectivity=True)
    # print('islands:', net['num_islands'])
    


# def runac(net, init):
#     check_net = deepcopy(net)
#     isolated, num_out_of_service, num_islands = _island_check(check_net)

#     if isolated and num_out_of_service > 0:
#         runpp(net, check_connectivity=True, init=init)
#     else:
#         runpp(net, check_connectivity=False, init=init)

#     net['num_islands'] = num_islands
#     net['num_out_of_service'] = num_out_of_service
