...
def _add_slack(net, ppc):
    ...
    def _ppci2ppc(ppc, ppci):
        for i, ppci_bus in enumerate(ppci['bus']):
            for j, ppc_bus in enumerate(ppc['bus']):
                if ppc_bus[BUS_I] == ppci_bus[BUS_I]:
                    ppc['bus'][j] = ppci_bus
                     
    # ppci_bus = ppc['bus'][~oos_busses]
    # bt = ppc["bus"][:, BUS_TYPE]
    ...
    # ppci_branch = ppc["branch"][brs]
    # ppci_bus[:, BUS_I] = ppc['bus'][:len(ppci_bus), BUS_I]
    ...
    # if np.any(~np.isfinite(ppc['gen'])):
    #     assert(False)
    ppci = _ppc2ppci(ppc, net)
    ppci_bus = ppci['bus']
    # if np.any(~np.isfinite(ppci_bus)):
    #     assert(False)

    B, _, _, _ = makeBdc(ppci['bus'], ppci['branch'])
    ...
    if conn_B[0] == 1:
        _ppci2ppc(ppc, ppci)
        return ppc['bus'], conn_B[0]

    for i in range(len(islands)):   # check the availability of each island
        # print('entered multi-island mode')
        if np.any(ppci_bus[islands[i][:], 1]==REF):    # if there is a slack bus in this island
            pass
        else:
            i_island = islands[i]
            if np.any(ppci_bus[i_island, 1] == PV) and np.any(ppci_bus[i_island, 2] > 0):   # generator > 0 and load > 0
                bus_flatten = ppci_bus[i_island, 1].flatten()
                ...
                ppci_bus[i_island[0][new_ref], 1] = REF  # reassign the slack bus here
                # np.argmax()
            else:   # if the island cannot work, let them be out of service
                ppci_bus[i_island, 1] = NONE
                pass
    _ppci2ppc(ppc, ppci)
    # if np.any(~np.isfinite(ppc['gen'])):
    #     assert(False)
...
def _pd2ppc(net, sequence=None):
    ...
    temp_net = deepcopy(net) # ! new
    temp_ppc = deepcopy(ppc) # ! new
    # add fake gen_ppc
    _build_gen_ppc(temp_net, temp_ppc) # ! new
    ppc['bus'], net['num_islands'] = _add_slack(temp_net, temp_ppc) # ! new
